...
int hio_init(void) {
    ...
    for (i = 0; i < N_FLOORS; ++i) {
        if (i != 0)
            hio_set_button_lamp(BUTTON_CALL_DOWN, i, 0);

        if (i != N_FLOORS - 1)
            hio_set_button_lamp(BUTTON_CALL_UP, i, 0);

        hio_set_button_lamp(BUTTON_COMMAND, i, 0);
    }

    // Clear stop lamp, door open lamp, and set floor indicator to ground floor.
    hio_set_stop_lamp(0);
    hio_set_door_open_lamp(0);
    hio_set_floor_indicator(0);
...
int hio_get_button_signal(hio_button_type_t button, int floor) {
	//stopper programmet ved ugyldig verdi
    assert(floor >= 0);
    assert(floor < N_FLOORS);
